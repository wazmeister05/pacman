...
public class MemetixGhosts extends Controller<EnumMap<GHOST,MOVE>> {	
	...
	private static final int			MAX_DISTANCE = 10;	//How many moves to look ahead
	...
	public EnumMap<GHOST,MOVE> getMove(Game game,long timeDue)
	{
		 ...
		 */
		if (!skip) {
			MoveIterator moves = new MoveIterator(game);
			while (moves.hasNext()) {
				MOVE[] m = moves.next();
				...
				if (highest < best) {
					best = highest;
					myMoves = testMoves;
					lastMove = myMoves; //In case we time out - store the current best set of moves
				}
			}
		}
	...
	private ArrayList<ArrayList<Integer>> walkMaze() {
		...
		while (ghostsEdge.size() > 0) { //Keep walking while we have somewhere to go
			 ...
			 */
			HashSet<ghostMove>	nextGhosts = new HashSet<ghostMove>();
			for (ghostMove gi: ghostsEdge) {
		...
		while (pacmanEdge.size() > 0) { //Keep walking while we have somewhere to go
			 ...
			 */
			HashSet<Integer>	nextPacman = new HashSet<Integer>();			
			for (int n: pacmanEdge)
	 ...
	 * Move away from the pacman (weighting 2)
	 * Move into unsafe zone (weighting 3)
	 */
	private int scoreEdible(GHOST g, ArrayList<ArrayList<Integer>> ev) {
		...
		if (nearest != Integer.MAX_VALUE && safe[game.getGhostCurrentNodeIndex(g)])
			score += 3*(int)nearest;
		...
		for (GHOST g: GHOST.values()) {
			if (eventHorizon.get(g.ordinal()).size() == 0 && game.getGhostLairTime(g) <= 0) {
				if (game.getGhostEdibleTime(g) > 0) { //Ghost is edible and in danger
					...
					if (containsPowerPill) //Move away from the pacman
